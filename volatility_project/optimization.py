...
def optimize_tspl_parameters(
    ...
    """
    # Default bounds: all parameters strictly positive, no upper bound
    if bounds is None:
        bounds = [
    ...
    def objective(params: np.ndarray) -> float:
        # ``model_class`` is a lambda so that cross_validate_tspl
        # instantiates a TSPLModel with the chosen smoothing_span
        ...
        try:
            ...
            # If the model fails to fit (e.g., no valid samples after dropping NaNs),
            # return a large penalty so that the optimiser avoids this region.
            return np.inf
